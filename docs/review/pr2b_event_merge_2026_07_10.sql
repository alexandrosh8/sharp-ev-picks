-- REVISION 2026-07-10 of pr2b_event_merge.sql: identical except the two
-- match_review_queue repoints are preceded by a twin-row dedupe (see inline
-- comments) — the 2026-07-10 run aborted on uq_match_review_queue_dedupe.
...
-- 2026-07-10 conflict fix: a fold-side queue row whose exact
-- (source, source_event_id, keep, reason) twin ALREADY exists on the keep
-- event cannot be repointed (uq_match_review_queue_dedupe) — the keep-side
-- row carries the same information, so the fold-side duplicate is deleted.
DELETE FROM match_review_queue q
USING _merge_map m
WHERE q.candidate_canonical_event_id = m.fold_id
  AND EXISTS (
    SELECT 1 FROM match_review_queue k
    WHERE k.source = q.source
      AND k.source_event_id = q.source_event_id
      AND k.candidate_canonical_event_id = m.keep_id
      AND k.reason = q.reason
  );
