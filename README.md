...
| -------------------------- | --- | ---------- | --------------------- | -------------------------------------------------------------------- |
| **Premium** (live default) | 62  | **+22.4%** | **+0.107** ( > 2 SE ) | positive even vs the Max-of-books close; 1X2 and OU2.5 each positive — max-book **gross** fill, i.i.d. SE (see caveats below) |
...
>
> **Backtest-honesty caveats (audit 2026-07-01) — read the headline as an upper bound, not the live expectation.** (1) _Fill universe:_ the numbers above were earned filling at football-data's **gross Max across ALL books** — exchanges included, gross of commission — while live fills at the best **soft** book, or an exchange **net** of commission. That fill is optimistic vs live. A soft-book-only variant (sharp Pinnacle never a fill; Betfair Exchange prices only net of commission) now exists: `uv run python scripts/value_backtest.py --fill-universe soft`. (2) _Correlation:_ the original "> 2 SE" read treated same-match 1X2 + O/U 2.5 picks as independent; they are correlated, so that SE was understated. The script's verdict now gates on a **cluster-robust (by-match) SE**, printed alongside the old i.i.d. SE. No re-anchored number is quoted here on purpose: the 2025 holdout is **spent** (ADR-0019 pre-registers the follow-ups), so the defensible soft-net, cluster-robust headline awaits a single-shot run on fresh 2026 data.
