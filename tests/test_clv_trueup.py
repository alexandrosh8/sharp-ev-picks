...
async def test_capture_oddschecker_results_writes_score_by_subevent_id(  # type: ignore[no-untyped-def]
    factory, monkeypatch
) -> None:
    # An OddsChecker-sourced FINISHED pick settles from OddsChecker's OWN results
    # feed: the capture posts our stored subevent id, and the returned FINAL score
    # is written to Event.scraped_* by EXACT id match (no cross-source name risk).
    from datetime import timedelta

    from app.clv_trueup import capture_oddschecker_results
    from app.ingestion.oddschecker import OddsCheckerResult
    from app.storage.models import Event

    event_id = "oddschecker:770001"
    async with factory() as session:
        from sqlalchemy import update as sa_update
    ...
    async def fake_fetch_results(subevent_ids, *, sport_key, proxy=None, **_):  # type: ignore[no-untyped-def]
        assert 770001 in subevent_ids and sport_key == "soccer"
        return [
            OddsCheckerResult(
                subevent_id=770001,
                prematch_subevent_id=None,
                home_name="Home FC",
                away_name="Away FC",
                home_score=3,
                away_score=0,
    ...
    monkeypatch.setattr("app.ingestion.oddschecker.fetch_results", fake_fetch_results)

    written = await capture_oddschecker_results(factory, "soccer", now=NOW)
    assert written == 1
    async with factory() as session:
        ev = await session.scalar(select(Event).where(Event.external_ref == event_id))
        assert ev is not None
        assert ev.scraped_home_score == 3
        assert ev.scraped_away_score == 0


async def test_capture_oddschecker_results_matches_prematch_id(factory, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    # Our stored id is the PRE-MATCH subevent id; the feed keys the result by the
    # live subeventId with preMatchSubeventId linking back. Match on either.
    from datetime import timedelta
    ...
    event_id = "oddschecker:880002"  # our stored (pre-match) id
    async with factory() as session:
        from sqlalchemy import update as sa_update
        ...
        await persist_pick(
            session,
            make_pick(event_id),
            EventTeams(home="H", away="A", starts_at=NOW - timedelta(hours=5)),
            "value-sharp-vs-soft",
            "v2-test",
        )
        await session.commit()
    ...
    async def fake_fetch_results(subevent_ids, *, sport_key, proxy=None, **_):  # type: ignore[no-untyped-def]
        return [
            OddsCheckerResult(
                subevent_id=999999,  # live id (not ours)
                prematch_subevent_id=880002,  # links to our stored id
                home_name="H",
                away_name="A",
                home_score=1,
                away_score=1,
    ...
    async with factory() as session:
        ev = await session.scalar(select(Event).where(Event.external_ref == event_id))
        assert ev is not None and ev.scraped_home_score == 1 and ev.scraped_away_score == 1
