...
async def test_anchor_restated_flag_derived_from_label_audit_table(session) -> None:  # type: ignore[no-untyped-def]
    """Live review 2026-08-02 (item 4): /picks rows carry anchor_restated=True
    exactly when the ops-script bookmaker_label_restatements audit table holds
    a (table_name='picks', column_name='anchor_book') row for the pick — and
    the lookup degrades to False (never an error) when the table is absent."""
    teams = EventTeams(home="Alpha FC", away="Beta United", league="test-league-persist")
    await persist_pick(
        session, make_pick("evt-anchor-restated"), teams, "value-sharp-vs-soft", "ar-1"
    )
    row = await session.scalar(select(Pick).where(Pick.bookmaker == "testbook"))
    assert row is not None

    def _ours(payload):  # type: ignore[no-untyped-def]
        return [p for p in payload if p["bookmaker"] == "testbook"][0]

    # The audit table may or may not exist on this warehouse (it is created by
    # the ops script, not Alembic). Drop it inside this rolled-back
    # transaction so the absent-table savepoint path is exercised for real.
    await session.execute(sa.text("DROP TABLE IF EXISTS bookmaker_label_restatements"))
    ours = _ours(await latest_picks_with_events(session, limit=200))
    assert ours["anchor_restated"] is False  # absent table -> honest False

    await session.execute(
        sa.text(
            "CREATE TABLE bookmaker_label_restatements ("
            " id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,"
            " table_name VARCHAR(64) NOT NULL,"
            " row_id BIGINT NOT NULL,"
            " column_name VARCHAR(64) NOT NULL,"
            " old_label VARCHAR(512), new_label VARCHAR(512),"
            " reason VARCHAR(128) NOT NULL,"
            " restated_at TIMESTAMPTZ NOT NULL DEFAULT now())"
        )
    )
    # A restatement of a DIFFERENT column/table never flags the pick.
    await session.execute(
        sa.text(
            "INSERT INTO bookmaker_label_restatements"
            " (table_name, row_id, column_name, old_label, new_label, reason) VALUES"
            " ('picks', :rid, 'bookmaker', 'Betfair Sportsbook', 'Betfair Exchange', 'test'),"
            " ('candidate_evaluations', :rid, 'anchor_book', 'Betfair Exchange', '10bet', 'test')"
        ),
        {"rid": row.id},
    )
    ours = _ours(await latest_picks_with_events(session, limit=200))
    assert ours["anchor_restated"] is False
    # The picks/anchor_book restatement flips the badge on.
    await session.execute(
        sa.text(
            "INSERT INTO bookmaker_label_restatements"
            " (table_name, row_id, column_name, old_label, new_label, reason) VALUES"
            " ('picks', :rid, 'anchor_book', 'Betfair Exchange', '10bet', 'test')"
        ),
        {"rid": row.id},
    )
    ours = _ours(await latest_picks_with_events(session, limit=200))
    assert ours["anchor_restated"] is True
