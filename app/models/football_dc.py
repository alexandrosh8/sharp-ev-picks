...
class DixonColesFootballModel:
    ...
    def fit(self, rows: Sequence[MatchRow], as_of: date) -> None:
        ...
        """
        usable = [r for r in rows if r.match_date <= as_of]
        if len(usable) < 50:
            raise ValueError(f"need >= 50 historical matches to fit, got {len(usable)}")

        from penaltyblog.models import DixonColesGoalModel

        weights = [math.exp(-self._xi * (as_of - r.match_date).days) for r in usable]
        model = DixonColesGoalModel(
            goals_home=[r.home_goals for r in usable],
            goals_away=[r.away_goals for r in usable],
            teams_home=[r.home_team for r in usable],
            teams_away=[r.away_team for r in usable],
            weights=weights,
        )
        model.fit(minimizer_options={"maxiter": 1000})
        self._model = model
        self._trained = {
            _normalize(team): team for r in usable for team in (r.home_team, r.away_team)
        }
        logger.info(
    ...
    async def predict(self, event_id: str) -> Sequence[PredictedProbability]:
        ...
        if teams is None:
            return ()
        home = self.resolve_team(teams.home)
        away = self.resolve_team(teams.away)
        if home is None or away is None:
            logger.debug(
                "team resolution miss for %s: %r -> %r, %r -> %r",
                event_id,
                teams.home,
                home,
                teams.away,
        ...
        try:
            grid = self._model.predict(home, away)
        ...
        return (
            PredictedProbability(Market.H2H, teams.home, float(grid.home_win), conf),
            PredictedProbability(Market.H2H, "Draw", float(grid.draw), conf),
            PredictedProbability(Market.H2H, teams.away, float(grid.away_win), conf),
