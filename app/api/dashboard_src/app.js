      ...
      (function () {
        ...
        function validateHealthPayload(value, httpStatus) {
          ...
          // Backend contract (routes.py GET /health): "ok" (200), "partial"
          // (200 — some sports degraded while capture still flows; downstream
          // dataIsStale() must NOT treat it as "ok") and "degraded" (503).
          // Audit 2026-07-27: rejecting "partial" here failed the whole
          // health card closed into a false "Could not verify" banner.
          if (health.status !== "ok" && health.status !== "partial" && health.status !== "degraded") {
            throw responseError("Health payload has an unknown status.", "SchemaError");
          }
          if ((httpStatus === 200 && health.status === "degraded") ||
              (httpStatus === 503 && health.status !== "degraded")) {
            throw responseError("Health HTTP status and payload disagree.", "SchemaError");
          }
          if (health.mode !== "picks-only") {
            throw responseError("Health payload is missing picks-only mode.", "SchemaError");
          }
          // Two legitimate shapes: the AUTHENTICATED body carries `polls` plus
          // interval/edge-floor detail; the redacted ANONYMOUS body (expired
          // session / public liveness) omits them and carries the boolean
          // has_completed_poll instead. Anything else still fails closed.
          const authenticatedDetail = isRecord(health.polls);
          if (!authenticatedDetail && typeof health.has_completed_poll !== "boolean") {
            throw responseError("Health payload is missing poll detail.", "SchemaError");
          }
          ...
          const requiredPositive = authenticatedDetail
            ? ["poll_interval_seconds", "max_odds_age_seconds", "poll_max_age_seconds"]
            : ["max_odds_age_seconds", "poll_max_age_seconds"];
        ...
        // Operator item 2 (2026-08-02): explicit "value gone" state. An OPEN
        // premium pick whose LIVE re-priced edge fails the qualifying-edge
        // gate (pick 946692: PREMIUM chip, edge −2.3%) must not lead with a
        // bare PREMIUM chip. Scoped to rows where the verdict is PROVABLE:
        // a finite re-priced current_edge and a finite tier floor — a
        // never-re-priced row is "awaiting re-price", not "value gone".
        // Display only: reuses hasQualifyingEdgeNow/edgeFloorOf verbatim;
        // isActionable and all gating are untouched. Settled / superseded /
        // started rows never match (status/kickoff guards below).
        function valueGone(p, health) {
          if (tierOf(p) !== "premium" || !p || p.status !== "alerted" || hasStarted(p)) return false;
          const edge = numOf(p.current_edge);
          const floor = edgeFloorOf(p, health);
          if (!Number.isFinite(edge) || !Number.isFinite(floor) || floor < 0) return false;
          return !hasQualifyingEdgeNow(p, health);
        }
        function valueGoneChipEl() {
          const t = document.createElement("span");
          t.className = "tag tag-warm tag-dashed";
          t.textContent = "VALUE GONE — no longer qualifies";
          t.title = "the live re-priced edge is below this pick's tier floor — the mint-time value is no longer available";
          return t;
        }
        // Operator item 2 (2026-08-04): "VALUE LOST <ago>" — the PERSISTED
        // transition timestamp (picks.value_lost_at, stamped by revalidation
        // when the re-priced edge first crossed below the tier floor). Purely
        // additive beside the VALUE GONE chip: chip logic stays untouched;
        // rows from before the column (or already re-qualified) return null.
        function valueLostAtEl(p) {
          if (!p || !p.value_lost_at) return null;
          const t = document.createElement("span");
          t.className = "tag tag-warm tag-dashed mono";
          t.textContent = "VALUE LOST " + fmtRelAge(p.value_lost_at);
          t.title = "value first fell below this pick's tier floor at " + fmtUtcTs(p.value_lost_at);
          return t;
        }
        ...
        // Defect C (fix 2026-08-02): while a pick is OPEN (not settled/
        // superseded) and no REAL close exists (closing_odds is null),
        // closing_fair_probability is only the LIVE re-priced fair — at mint it
        // equals model_probability, so the tautology read labelled every fresh
        // open pick "Tautological Close Excluded". Pre-close, the honest
        // provenance state is "no close recorded"; exclusion chips render only
        // once a real close exists or the pick is closed. DISPLAY-STATE ONLY:
        // isTrustedClv/clvExclusionOf (the trusted-CLV gating) are unchanged.
        function closePending(p) {
          return p.status !== "settled" && p.status !== "superseded" && p.closing_odds == null;
        }
        ...
        function clvStateLabel(p) {
          if (closePending(p)) return "—"; // open + no real close: renders "no close recorded"
        ...
        function edgeRowEl(p, health) {
          ...
          // Operator item 2 (2026-08-02): the "value gone" chip sits ADJACENT
          // to the tier chip so a failing open premium row never leads with a
          // bare PREMIUM chip. Display only.
          if (valueGone(p, health)) {
            // Item 2 (2026-08-04): the persisted "VALUE LOST <ago>" stamp sits
            // right after the chip when the transition timestamp exists.
            const goneChips = [valueGoneChipEl()];
            const lostAt = valueLostAtEl(p);
            if (lostAt) goneChips.push(lostAt);
            row.append(ko, ev, tier, ...goneChips, sel, trust);
          } else row.append(ko, ev, tier, sel, trust);
        ...
        function renderEdgeDetail() {
          ...
          // Operator item 2 (2026-08-02): same "value gone" chip beside the
          // drawer's tier chip — one shared predicate, both render sites.
          if (valueGone(p, state.health)) {
            head.appendChild(valueGoneChipEl());
            // Item 2 (2026-08-04): persisted transition time beside the chip.
            const lostAt = valueLostAtEl(p);
            if (lostAt) head.appendChild(lostAt);
          }
          ...
          if (!closePending(p) && p.clv_log != null && !clvExclusionOf(p)) {
