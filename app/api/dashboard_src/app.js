      ...
      (function () {
        ...
        // Audit 2026-07-09: mirror the backend trusted-subset gate
        // (_settled_close_is_trusted, app/storage/repositories.py) as far as
        // the /picks row exposes it: a MEASURED clv_log, a sharp close anchor,
        // independence exactly true, and no honesty exclusion (tautological/
        // circular/fabricated). has_snapshot_close and the devig-fallback
        // flags are NOT serialized on /picks — close_independent_of_fill is
        // only ever stamped from a genuine snapshot close (its closest exposed
        // provenance), and the backend treats missing devig provenance as
        // symmetric, so omitting both here matches its conservative reading.
        ...
        function clvIsFabricated(p) {
          ...
          if (isFinite(d) && d > 0 && isFinite(cf)) {
            ...
            return cf - 1 / d > CLOSE_IMPLIED_EDGE_CEILING;
          }
